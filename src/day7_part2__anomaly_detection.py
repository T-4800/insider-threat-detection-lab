...
def detect_anomalies(records, baseline):
    ...
    for r in records:
        distance = ( 
            abs(r["failed_logins"] - baseline["avg_failed_logins"]) * 1
            abs(r["restricted_downloads"] - baseline["avg_restricted_downloads"]) * 3 
        )
...
def print_anomaly_report(anomalies):
    print("\nAnomaly ranking (most unusual first):")
    sorted_users = sorted(anomalies, key=lambda x: x["distance"], reverse=True)
